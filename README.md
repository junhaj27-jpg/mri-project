...
| 구조별 3D 모델 생성 | SynthSeg/FastSurfer label map에서 대뇌, 소뇌, 뇌간, 해마 mask와 GLB 생성 |
...
NIfTI mask 파일을 직접 읽어 spacing 기반 부피를 계산할 수도 있습니다.

```bash
curl -X POST http://127.0.0.1:8000/api/analysis/volume/nifti ^
  -H "Content-Type: application/json" ^
  -d "{\"study_label\":\"T08\",\"mask_nifti_path\":\"raw_private/P001/T08/tumor_mask.nii.gz\"}"
```

## 구조별 3D 모델 생성 API

원본 MRI를 바로 3D mesh로 변환하면 두피, 두개골, 뇌 전체가 함께 잡힐 수 있습니다. 대뇌, 소뇌, 뇌간, 해마처럼 구조별로 구분하려면 먼저 SynthSeg, FastSurfer, FreeSurfer 등 외부 구조 분할 도구로 segmentation label map을 생성해야 합니다.

```bash
curl -X POST http://127.0.0.1:8000/api/analysis/structure-mesh ^
  -H "Content-Type: application/json" ^
  -d "{\"study_label\":\"T08\",\"patient_code\":\"P001\",\"seg_nifti_path\":\"raw_private/P001/T08/synthseg_seg.nii.gz\"}"
```

이 API는 label map에서 구조별 mask를 분리하고 다음 경로에 GLB를 생성합니다.

```text
media/models/P001/T08/structures/cerebrum.glb
media/models/P001/T08/structures/cerebellum.glb
media/models/P001/T08/structures/brainstem.glb
media/models/P001/T08/structures/hippocampus.glb
```

현재 label 번호는 FreeSurfer/SynthSeg aseg 스타일 예시 기준이며, 실제 사용하는 분할 도구의 LUT에 맞춰 `backend/services/structure_masks.py`에서 조정해야 합니다.
...
4. SynthSeg/FastSurfer 등으로 구조 분할 label map을 생성합니다.
5. 병변 mask 또는 구조별 mask를 생성하거나 외부 툴에서 가져옵니다.
6. mask voxel 개수와 spacing 정보로 부피를 계산합니다.
7. mask를 marching cubes로 GLB mesh로 변환합니다.
8. `media/models/P001/Txx/`에 병변 또는 구조별 GLB를 저장합니다.
9. 웹에서 2D preview, overlay, 3D 모델, 부피 변화를 확인합니다.
...
- 원본 MRI 폴더는 웹 정적 경로로 공개하지 않습니다.
- 공개 저장소에는 코드, 샘플 메타데이터, 샘플 결과 JSON, 공개 가능한 일러스트 자산만 포함합니다.
